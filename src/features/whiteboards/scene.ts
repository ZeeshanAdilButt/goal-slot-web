...
export function buildScene(
  ...
  const {
    ...
    openPopup: _p,
    contextMenu: _ctx,
    // Viewport-derived and transient session state: persisting these restored a
    // stale canvas size / welcome-screen state on the next open. Added on main
    // (PR #241 era) to the in-component buildScene; kept here now that this
    // function is the single persist path.
    width: _w,
    height: _h,
    offsetTop: _ot,
    offsetLeft: _ol,
