...
export function SchedulePage() {
  ...
  // Stable across renders (useCallback) so ScheduleGrid/DraggableBlock — which are memoized —
  // don't re-render just because SchedulePage re-rendered for an unrelated reason.
  const handleEdit = useCallback((block: ScheduleBlock) => {
  ...
  const handleViewDetail = useCallback((block: ScheduleBlock) => {
  ...
  const handleAddBlock = useCallback((dayOfWeek: number, preset?: { startTime: string; endTime: string }) => {
  ...
  return (
    <PageShell width="wide" className="isolate">
      ...
      {/* Active block call-out — mirrors the FocusNowBar so the schedule
          ...
          the block detail dialog. Hidden during free time. Lives in its
          own component (see below) so its 30s ticker only re-renders the
          banner, not the whole page + schedule grid. */}
      <ActiveNowBanner weekSchedule={weekSchedule} />
...
// Owns its own 30s ticker so only this small banner re-renders on tick —
// previously `now` lived in SchedulePage, so every 30s the whole page
// (including the full ScheduleGrid + every block) re-rendered just to
// keep this banner's "current time" comparison fresh.
function ActiveNowBanner({ weekSchedule }: { weekSchedule: WeekSchedule }) {
  ...
  const activeBlock = useMemo(() => findScheduleBlockForDateTime(weekSchedule, now), [weekSchedule, now])
    ...
    <div className="relative overflow-hidden rounded-xl border border-[#f2cc0d]/40 bg-gradient-to-r from-[#fffbea] via-[#fffbea] to-[#fff7d1] px-4 py-3 shadow-sm">
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5">
        <div className="flex shrink-0 items-center gap-2">
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#8a7307]">Active now</span>
        </div>
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-1.5">
          ...
          <Button type="button" variant="secondary" size="sm" onClick={() => scrollToBlock(activeBlock.id)}>
